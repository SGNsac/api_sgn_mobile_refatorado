...
export class ApprovalRequestService {
  public async execute(
    ...
    try {
      ...
      const valCountNumAprovaPedidoBD = await sql.query(
        selectAprovaPedidoQuery
      );

      if (
        valCountNumAprovaPedidoBD.recordset[0].PAGE_TODAS_APROVACOES_PEDIDO ===
        'S'
      ) {
        if (
          valCountNumAprovaPedidoBD.recordset[0].PAGE_NUM_APROVACOES_PEDIDO ===
          1
        ) {
          ...
          valCountNumAprovaPedidoBD.recordset[0].PAGE_NUM_APROVACOES_PEDIDO ===
            2 &&
          valCountNumAprovaPedido.recordset[0].NUM === 1
        ) {
          sqlQuery = 'PEDI_STATUS = \'A\',';
        } else if (
          valCountNumAprovaPedidoBD.recordset[0].PAGE_NUM_APROVACOES_PEDIDO ===
            3 &&
          valCountNumAprovaPedido.recordset[0].NUM === 2
        ) {
          sqlQuery = 'PEDI_STATUS = \'A\',';
        } else if (
          valCountNumAprovaPedidoBD.recordset[0].PAGE_NUM_APROVACOES_PEDIDO ===
            4 &&
          valCountNumAprovaPedido.recordset[0].NUM === 3
        ) {
          sqlQuery = 'PEDI_STATUS = \'A\',';
        }
      } else {
