...
export class ListCodServiceContractAdditive {
  public async execute (token: string, ADCS_COCS_COD:string):Promise<IAdcs[]> {
    ...
    `
    const sql = selectAditivoContrato1(cod + '', queryString)
    const sql2 = selectAditivoContrato2(cod + '', queryString)
    const sql3 = selectAditivoContrato3(cod + '', queryString)
    const sql4 = selectAditivoContrato4(cod + '', queryString)
